...
(function () {
  ...
  function runLines(aText, bText) {
    ...
    if (script === null) {
      els.out.innerHTML =
        '<div class="gap">these two are too different to line up — ' +
    ...
    els.tally.innerHTML =
      '<span class="tag ok">+' + adds + '</span> ' + '<span class="tag err">−' + dels + '</span>';
    if (!adds && !dels)
      status.ok(
        aText === bText ? 'identical' : 'identical, once the ignored differences are set aside'
      );
  ...
  function runWords(aText, bText) {
    ...
    script.forEach(function (e) {
      if (e.op === 0) html += ui.esc(A[e.ai]);
        ...
        adds++;
        html += '<ins>' + ui.esc(Bt[e.bi]) + '</ins>';
      } else {
        dels++;
        html += '<del>' + ui.esc(A[e.ai]) + '</del>';
      }
    });
  ...
  function tokens(t) {
    return t.split(/(\s+|\b)/).filter(function (s) {
      return s !== '';
    });
  }
    ...
    groups.forEach(function (g, gi) {
      ...
      chunk(script.slice(g.from, g.to)).forEach(function (c) {
        ...
        if (layout === 'split') {
          ...
          pairs.forEach(function (p) {
            if (p.del)
              html += uniRow(p.del.ai, null, 'del', p.mark ? p.mark.a : ui.esc(A[p.del.ai]));
          });
          pairs.forEach(function (p) {
            if (p.add)
              html += uniRow(null, p.add.bi, 'add', p.mark ? p.mark.b : ui.esc(Bl[p.add.bi]));
          });
        }
      });
    });
  ...
  function gapRow(n) {
    return n > 0
      ? '<div class="gap">⋯ ' + n + ' unchanged ' + (n === 1 ? 'line' : 'lines') + '</div>'
      : '';
  }
  ...
  function buildPatch(script, A, Bl) {
    ...
    groups.forEach(function (g) {
      ...
      lines.push(
        '@@ -' +
          (aStart === null ? 0 : aStart + 1) +
          ',' +
          aCount +
          ' +' +
          (bStart === null ? 0 : bStart + 1) +
          ',' +
          bCount +
          ' @@'
      );
