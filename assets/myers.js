...
SK.myers = (function () {
  ...
  function diff(a, b, maxD) {
    ...
    var mid = search(a.slice(pre, n - post), b.slice(pre, m - post),
  ...
  function search(a, b, maxD) {
    var n = a.length, m = b.length;
    if (!n && !m) return [];
    if (!n) return b.map(function (_, i) { return { op: 1, ai: null, bi: i }; });
    if (!m) return a.map(function (_, i) { return { op: -1, ai: i, bi: null }; });
  ...
  function backtrack(trace, d, n, m) {
    ...
    for (; d > 0; d--) {
      ...
      while (x > xMid && y > yMid) { x--; y--; out.push({ op: 0, ai: x, bi: y }); }
      if (down) { y--; out.push({ op: 1, ai: null, bi: y }); }
      else { x--; out.push({ op: -1, ai: x, bi: null }); }
      x = xStart; y = yStart;
    }
    while (x > 0 && y > 0) { x--; y--; out.push({ op: 0, ai: x, bi: y }); }
